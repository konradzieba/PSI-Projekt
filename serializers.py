...
class AddressSerializer(serializers.ModelSerializer):
    class Meta:
        model = Address
        fields = ['url', 'id', 'zip_code', 'town', 'country', 'street', 'apartment_number']
...
class RentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data['rent_start_date'] > data['rent_end_date']:
            raise serializers.ValidationError({'rent_end_date': 'Rent end date can not be before Rent start date'})
        return data
